...
""")
    ...
    subprocess.run(["git", "commit", "-m", "Add dependencies and test infrastructure"],
...
"""
    ...
    subprocess.run(["git", "commit", "-m", f"Modify {filename} - {random_suffix}"],
                  cwd=repo_path, check=True)
...
def test_worktree_temporary_branch_swap(repo_with_deps):
    ...
    subprocess.run(
        ["git", "branch", "temp-mc-feature", "mc-feature"],
        cwd=repo_path,
        check=True
    )
    ...
    worktree_path_new = create_worktree(repo_path, "mc-feature", "mc-feature-restored", create_new_branch=False)
...
def test_bare_clone_as_hub(repo_with_deps):
    ...
    subprocess.run(
        ["git", "clone", str(bare_path), str(main_clone)],
        check=True
    )
    ...
    subprocess.run(
        ["git", "clone", str(bare_path), str(task_clone)],
        check=True
    )
...
def test_mirror_clone_approach(repo_with_deps):
    ...
    subprocess.run(
        ["git", "clone", "--mirror", str(repo_path), str(mirror_path)],
        check=True
    )
    ...
    subprocess.run(
        ["git", "clone", str(mirror_path), str(work_clone)],
        check=True
    )
    ...
    subprocess.run(
        ["git", "clone", "--bare", str(repo_path), str(regular_bare)],
        check=True
    )
...
def test_direct_clone_to_clone(repo_with_deps):
    ...
    subprocess.run(
        ["git", "clone", str(main_repo), str(task_clone)],
        check=True
    )
