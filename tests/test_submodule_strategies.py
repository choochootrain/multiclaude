...
""")
    ...
    subprocess.run(
        ["git", "-c", "protocol.file.allow=always", "submodule", "add", str(lib_repo), "libs/shared-lib"],
...
""")
    ...
    subprocess.run(["git", "commit", "-m", "Add main app with submodule"], cwd=main_repo, check=True)
    
    return main_repo, lib_repo

# Helper functions

def create_worktree(repo_path: Path, branch_name: str, worktree_name: str = None, create_new_branch: bool = True) -> Path:
...
def setup_submodule_branch(repo_path: Path, submodule_path: str, branch_name: str):
    ...
    subprocess.run(
        ...
        cwd=submodule_full_path, check=True, capture_output=True
    )
...
""")
    ...
    subprocess.run(
        ...
        cwd=submodule_full_path, check=True
    )
...
def verify_submodule_state(repo_path: Path, submodule_path: str, expected_branch: str = None):
    ...
    if expected_branch and current_branch:
        assert current_branch == expected_branch, f"Expected branch {expected_branch}, got {current_branch}"
...
def test_worktree_temporary_branch_swap_with_submodules_fails(repo_with_submodules):
    ...
    subprocess.run(["git", "config", "protocol.file.allow", "always"], cwd=worktree_path, check=True)
    subprocess.run(
    ...
    worktree_branch, worktree_commit = verify_submodule_state(worktree_path, "libs/shared-lib", branch_name)
...
def test_worktree_patch_based_swap_with_submodules_fails(repo_with_submodules):
    ...
    if submodule_log.stdout.strip():
        ...
        subprocess.run(
            ["git", "format-patch", f"-{num_submodule_commits}", "HEAD", "-o", str(submodule_patches_dir)],
            cwd=worktree_submodule,
            capture_output=True,
            check=True
        )
...
def test_mirror_clone_with_submodules(repo_with_submodules):
    ...
    subprocess.run(
        ["git", "clone", "--mirror", str(main_repo), str(mirror_path)],
        check=True
    )
    ...
    subprocess.run(
        ["git", "-c", "protocol.file.allow=always", "clone", "--recursive", str(mirror_path), str(task_repo)],
        check=True
    )
    ...
    subprocess.run(
        ...
        cwd=task_submodule, check=True
    )
    
    # Clone from mirror to main work repo
    main_work = base_dir / "main-from-mirror"
    subprocess.run(
        ["git", "-c", "protocol.file.allow=always", "clone", "--recursive", str(mirror_path), str(main_work)],
        check=True
    )
    ...
    subprocess.run(
        ["git", "-c", "protocol.file.allow=always", "submodule", "update", "--recursive", "--remote"],
        cwd=main_work, check=True, capture_output=True
    )
    ...
    subprocess.run(
        ...
        cwd=main_work_submodule, check=True, capture_output=True
    )
...
def test_direct_clone_to_clone_with_submodules(repo_with_submodules):
    ...
    subprocess.run(
        ["git", "-c", "protocol.file.allow=always", "clone", "--recursive", str(main_work), str(task_repo)],
        check=True
    )
